...
template <int kUpdateIters, int kFinalizeIters> class SipHash {
  public:
  ...
  private:
    template <size_t rounds> INLINE void compress() {
...
template <class State> void update_state(const char *bytes, const U64 size, State *state) {
    // Feed entire packets.
...
template <class State> U64 compute_hash(const U64 key[2], const char *bytes, const U64 size) {
    State state(key);
